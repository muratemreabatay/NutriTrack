...
const WaterTrackerScreen = () => {
    ...
    const getMessage = () => {
        if (waterGlasses === 0) return t.water.messages.start;
        if (progress < 0.25) return t.water.messages.low;
        if (progress < 0.5) return t.water.messages.mid;
        if (progress < 0.75) return t.water.messages.high;
        if (progress < 1) return t.water.messages.almost;
        return t.water.messages.done;
    };
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#030712' }}>
            ...
            >
                <Text className="text-white text-2xl font-bold">{t.water.title}</Text>
            ...
            >
                <View className="relative w-[220px] h-[220px] items-center justify-center">
                    <Svg height="220" width="220" viewBox="0 0 220 220" style={{ position: 'absolute' }}>
                        <Circle cx="110" cy="110" r={radius} stroke="#1E3A5F" strokeWidth={strokeWidth} fill="transparent" />
                        <Circle cx="110" cy="110" r={radius}
                            stroke="#38BDF8" strokeWidth={strokeWidth} fill="transparent"
                    ...
                    <View className="items-center">
                        ...
                        <Text className="text-gray-400 text-xs">/ {waterTarget} {t.water.glasses}</Text>
                    </View>
                </View>
            </Animated.View>
            ...
            >
                <View className="flex-1 items-center">
                    ...
                    <Text className="text-gray-500 text-xs mt-1">{t.water.mlDrank}</Text>
                </View>
                <View className="w-px bg-gray-800" />
                <View className="flex-1 items-center">
                    <Text className="text-gray-300 text-xl font-bold">{Math.max(0, mlTarget - mlConsumed)}</Text>
                    <Text className="text-gray-500 text-xs mt-1">{t.water.mlLeft}</Text>
                </View>
            </Animated.View>
            ...
            <View className="absolute bottom-10 left-6 right-6">
                <View className="flex-row items-center justify-center" style={{ gap: 16 }}>
                    <TouchableOpacity
                        onPress={handleRemove} disabled={waterGlasses === 0} activeOpacity={0.7}
                    ...
                    <TouchableOpacity
                        onPress={handleAdd} activeOpacity={0.8}
